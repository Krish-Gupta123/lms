...
import BorrowalRequestsTable from "./BorrowalRequestsTable";
...
const TABLE_HEAD = [
  ...
  { id: "penalty", label: "Penalty (₹)", alignRight: false },
...
const BorrowalPage = () => {
  ...
  return (<>
    ...
    <Container>
      {user && user.isAdmin && <BorrowalRequestsTable />}
              ...
              {borrowals.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((borrowal) => (
                <TableRow hover key={borrowal._id} tabIndex={-1}>
                  ...
                  <TableCell align="left">
                    {(borrowal.penalty && borrowal.penalty > 0)
                      ? <Label color="error" sx={{padding: 2}}>₹{borrowal.penalty}</Label>
