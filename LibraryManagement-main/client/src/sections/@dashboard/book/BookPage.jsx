      ...
      <Container>
        ...
        {isTableLoading ? <Grid padding={2} style={{"textAlign": "center"}}><CircularProgress/></Grid> :
          ...
          books.length > 0 ? <Grid container spacing={4}>
            {books.map((book) => {
                console.log("Book image URL:", book.photoUrl); 
                return (
              <Grid key={book._id} item xs={12} sm={6} md={4}>
                <Card>
                  <Box sx={{pt: '80%', position: 'relative'}}>
                    ...
                    <StyledBookImage alt={book.name} src={`/static/mock-images/books/book1.jpg`}/>
                  </Box>

                  <Stack spacing={1} sx={{p: 2}}>
                    <Typography textAlign="center" variant="h5" margin={0} noWrap>{book.name}</Typography>
                    <Typography variant="subtitle1" sx={{color: "#888888"}} paddingBottom={1} noWrap textAlign="center">{book.author.name}</Typography>
                    ...
                    {/* Request to Borrow Button for users */}
