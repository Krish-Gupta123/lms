...
## Where to check JWT tokens

- **Login endpoint:**
  - Send a POST request to `/api/auth/jwt-login` with `{ email, password }` in the body.
  - The response will contain a `token` field. This is your JWT.

- **Protected endpoint:**
  - Send a GET request to `/api/auth/jwt-protected` with the header `Authorization: Bearer <your_token>`.
  - If the token is valid, you will get a success response with user info.

- **Token validation:**
  - All JWT validation and edge cases (missing, expired, invalid, tampered) are handled in `server/middleware/jwtAuth.js`.

- **Token structure:**
  - The JWT contains user id, email, role, and name.

You can use tools like [jwt.io](https://jwt.io/) to decode and inspect your JWT tokens.
